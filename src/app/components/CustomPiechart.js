...
//   return (
//     <div className="pie-chart relative flex flex-col w-full items-center justify-center mx-auto mt-12">
//       <svg ref={svgRef} width={"200"} height={"200"}></svg>
//       {tooltip.display && (
//         <div
//           className="tooltip bg-white p-2 rounded-md"
//           style={{
//             position: "absolute",
//             left: tooltip.x + "px",
//             top: tooltip.y + "px",
...
//         </div>
//       )}
//       <div className="legend w-full mt-4 space-y-2">
//         {data.map((item) => (
//           <div
//             key={item.label}
...
const PieChart = ({ data }) => {
  ...
  return (
    <div className="pie-chart flex flex-col w-full items-center justify-center mx-auto mt-12 ">
      <svg ref={svgRef} width={"200"} height={"200"}></svg>
      {tooltip.display && (
        <div
          className="tooltip bg-white p-2 rounded-md"
      ...
      <div className="legend w-full mt-4 space-y-2">
        {data?.map((item) => (
          <div
            key={item.label}
            className="legend-item flex items-center justify-between gap-4"
          >
            <div className="flex gap-x-2 items-center">
              <div
                ...
                style={{ backgroundColor: legendColorScale(item.label) }}
              ></div>
              <div className="label text-white">{item.label}</div>
            </div>

            <div className="value text-white">{item.value}</div>
          </div>
        ))}
      </div>
    </div>
