            ...
            <>
              {data?.map((row, index) => {
                return (
                  <div
                    key={index}
                    className={`ROW flex items-center gap-x-4 dark:text-[#F0EDFF] text-[#3a3a3a] text-[12px] font-bold border-b py-4 justify-between w-fit min-w-full break-words px-4 ${
                      redTable ? "px-4" : ""
                    } `}
                  >
                    {headers.map((header, index) => {
