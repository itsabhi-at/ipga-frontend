...
const AutoCompleteDropdownV2 = ({
  ...
  const handleSelect = (value, id, city, state) => {
    setIsSelectedFromDropdown(true);
    setInputValue(value);
    setSuggestions([]);
    setOpenSuggestion(false);
    onSelect(value, id, city, state);
  };
  const handleBlur = () => {
  ...
  return (
    <div className={`relative w-full space-y-2`}>
      <label htmlFor={htmlFor} className="text-[14px] text-white">
        {labelText}
      ...
      <div className="relative">
        <input
          ...
          //   onBlur={handleBlur}
          ref={inputRef}
          placeholder="Input 3 digits to search..."
        />
        <Image
          height={28}
          width={28}
