...
//   }, [inputValue]);
//   return (
//     <div className={`relative ${product ? "w-60" : "w-20"}`}>
//       <input
//         // className={`rounded-md bg-[#EE3524] bg-opacity-10 text-sm border-b-2 border-[#EE3524]`}
//         className="rounded-sm outline-[#E12228] w-full border-b p-2 pl-0 pr-8"
//         type="text"
//         value={inputValue}
...
//           <ul className=" absolute z-20 top-8 left-0 max-h-40 overflow-scroll rounded-b-md bg-[#fff]  text-sm border-2 border-opacity-30 border-[#EE3524] w-full">
//             {suggestions.map((option) => (
//               <li
//                 key={product ? option.id : option}
//                 className="text-[14px] font-light  text-[#0B1525] p-2 hover:bg-red-300 focus:bg-red-300 cursor-pointer "
//                 onClick={() =>
//                   handleSelect(
//                     product ? option.name : option,
//                     product ? option.id : option
//                   )
//                 }
//               >
//                 {product ? option.name : option}
...
import { AiOutlineCaretDown } from "react-icons/ai";
import { toast } from "react-toastify";
import { useGetCallUniversalMutation } from "../service/universalApi";
import Image from "next/image";

const AutoCompleteSiel = ({
  labelText,
  htmlFor,
  placeholderImage,
  isFieldRequired,
  ...
  const [openSuggestion, setOpenSuggestion] = useState(false);
  // mutation for data
  const [getCallMutation] = useGetCallUniversalMutation();
  // const [getCallMutation] =
  const handleChange = (event) => {
    const value = event.target.value;
    setInputValue(value);
    if (inputThree) {
    ...
    await getCallMutation({
      ...
      .then((res) => {
        if (res.status == "success") {
          switch (keyword) {
            case "ASM":
              setData(res.data.asm);
              break;
            case "RMM":
              setData(res.data.rmm);
              break;
            case "ZSM":
              setData(res.data.zsm);
              break;
          ...
          <ul className="overflow-auto absolute z-20 top-20 left-0 max-h-40 rounded-b-md bg-[#fff]  text-sm border-2 border-opacity-30 border-[#EE3524] w-full">
            {suggestions.map((option) => (
              <li
                key={option.id}
                className="text-[14px] font-light  text-[#0B1525] p-2 hover:bg-red-300 focus:bg-red-300 cursor-pointer "
                onClick={() => handleSelect(option.email, option.id)}
              >
                {option.email}
              </li>
            ))}
...
export default AutoCompleteSiel;
